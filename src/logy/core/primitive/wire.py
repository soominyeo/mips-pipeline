...
class Wire(Generic[D, D1, D2], Element[WireBehavior], classifier="W"):
    def __init__(self, data: D, pin_ins: Iterable[Tuple[Pin[D1], int]], pin_outs: Iterable[Tuple[Pin[D2], int]],
                 name: str = None):
        super().__init__(name=name)
        self.__delay: Dict[PinEntry, int] = {PinEntry(pin, mode): delay for pins, mode in
                                             [(pin_ins, Mode.IN), (pin_outs, Mode.IN)] for pin, delay in
                                             pins}
...
class SimpleWire(Generic[D], Wire[D, D, D], ABC, classifier="s"):
    def __init__(self, data: D, pin_ins: Iterable[Tuple[Pin[D1], int]], pin_outs: Iterable[Tuple[Pin[D2], int]],
        ...
        super(SimpleWire, self).__init__(data, pin_ins, pin_outs, name=name)

    @classmethod
    def direct(cls, start: Pin[D], end: Pin[D], delay: int = 0, name: str = None):
        wire = SimpleWire(start.data.of(None), [(start, delay)], [(end, 0)],
                          name=(name or f"[{start.name}>>{end.name}]"))
        return wire

    @classmethod
    def branch(cls, start: Pin[D], ends: Iterable[Tuple[Pin[D], int]], name: str = None):
        wire = SimpleWire(start.data.of(None), [(start, 0)], [(pin, delay) for pin, delay in ends],
                          name=(name or f"{start.name}>|[{next(iter(ends))}...]"))
        return wire
